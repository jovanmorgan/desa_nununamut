...
if (empty($id_oprator_musdous) || empty($username) || empty($password) || empty($nama)) {
    echo "data_tidak_lengkap";
    exit();
}

// Cek apakah username sudah ada di database
$check_query = "SELECT * FROM oprator_musdous WHERE username = '$username' AND id_oprator_musdous != $id_oprator_musdous";
...
// Cek apakah username sudah ada di database
$check_query_masyarakat = "SELECT * FROM masyarakat WHERE username = '$username'";
$result_masyarakat = mysqli_query($koneksi, $check_query_masyarakat);
if (mysqli_num_rows($result_masyarakat) > 0) {
    echo "error_username_exists"; // Kirim respon "error_email_exists" jika email sudah terdaftar
    exit();
}
// Cek apakah username sudah ada di database
...
$query_update = "UPDATE oprator_musdous SET username = '$username', password = '$password', nama = '$nama' WHERE id_oprator_musdous = '$id_oprator_musdous'";
